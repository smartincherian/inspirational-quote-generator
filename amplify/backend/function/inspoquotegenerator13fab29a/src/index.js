...
// 	API_INSPOQUOTEGENERATOR_GRAPHQLAPIIDOUTPUT
// 	API_INSPOQUOTEGENERATOR_QUOTEAPPDATATABLE_ARN
// 	API_INSPOQUOTEGENERATOR_QUOTEAPPDATATABLE_NAME
// 	ENV
...
async function updateQuoteDDBObject() {
  const quoteTableName =
    process.env.API_INSPOQUOTEGENERATOR_QUOTEAPPDATATABLE_NAME;
  const quoteObjectID = "123";
  ...
  async function getRandomQuote(apiURLInput) {
    ...
    const svgImage = `
          <svg width="${width}" height="${height}">
              ...
              <text x="${width / 2}" y="${
      height - 10
    }" class="footerStyles">Developed by @SmartinKCherian | Quotes from ZenQuotes.io</text>
          </svg>
        `;
